...
class UnoPlugin(Plugin):
    ...
    def show_game_state(self, game, event):
        ...
        event.send_message(cur_player.peer, str(cur_player.hand), as_notice=True)
    ...
    def setup_game(self, event):
        if not event.channel:
            event.reply("Sorry, I can only start a game in a channel.")
            return

        if self.game:
            # TODO help
            event.reply("There's already a game running.")
            return
    ...
    def start_game(self, event):
        with uno_error_catcher(event):
            ...
            self.show_game_state(game, event)
